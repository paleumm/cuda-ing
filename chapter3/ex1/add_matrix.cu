...
__global__ void add_mat_kernel_3(float *a, float *b, float *c, int n){
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if(index < n*n){
        for (size_t i = 0; i < n; i++){
            int offset = n * i + index;
...
__host__ void add_matrix(float *h_out, float *h_first, float *h_second, int n, int mode){
    ...
    {
    case 1:
        add_mat_kernel_1<<<ceil(dim/1024.0),1024>>>(d_first, d_second, d_out, n);
        break;
    case 2:
        add_mat_kernel_2<<<ceil(dim/1024.0),1024>>>(d_first, d_second, d_out, n);
        break;
    case 3:
        add_mat_kernel_3<<<ceil(dim/1024.0),1024>>>(d_first, d_second, d_out, n);
        break;
    default:
        add_mat_kernel_0<<<ceil(dim/1024.0),1024>>>(d_first, d_second, d_out, n);
        break;
    }
